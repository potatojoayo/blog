...
const SideMenu: React.FC = () => {
  ...
  const isNotifierOpen = useSelector(
    (state: RootState) => state.notifierState
  ).isOpen;
  const theme = themeState.theme;
  const isDark = themeState.isDark;
  const windowSize = useSelector(
    (state: RootState) => state.windowSizeState
  ).displaySize;
  ...
  return (
    <Container
      ...
      onClick={(e) => {
        e.stopPropagation();
        if (isNotifierOpen) dispatch(toggleNotifier());
      }}
      mobile={{
        position: "fixed",
        height: "100%",
    ...
    >
      ...
      >
        ...
        <Button>
          <Image
            ...
            onClick={(e) => {
              ...
              window
                .open(
                  "https://www.linkedin.com/in/효범-한-111156210/",
                  "_blank"
                )!
                .focus();
            }}
        ...
        <Container width="5px" />
        <Button display="flex" justifyContent="center" alignItems="start">
          <Image
            width={30}
            height={30}
            mobile={{
      ...
      />
      <MenuItem icon={i_study} name="STUDY" index={0} numChild={3}>
        <Link
          to="/list/front-end?page=1"
      ...
      <Link
        to="/list/article?page=1"
        style={{ textDecoration: "none", width: "100%" }}
        replace
      >
        <MenuItem icon={i_article} name="ARTICLE" index={1} numChild={0} />
      </Link>
      <Link
        to="/list/works?page=1"
        style={{ textDecoration: "none", width: "100%" }}
        replace
      >
        <MenuItem icon={i_works} name="WORKS" index={2} numChild={0} />
      </Link>
      <Link
        to="/list/tags"
        style={{ textDecoration: "none", width: "100%" }}
        replace
      >
        <MenuItem icon={i_tag} name="TAGS" index={3} numChild={0} />
      </Link>
      <Container
        position="absolute"
        bottom="20px"
      ...
      >
        <Text
          ...
          fontSize={12}
        >
          © 2021 potatojoayo All rights reserved
        </Text>
      </Container>
    </Container>
  );
